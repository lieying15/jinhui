package com.thlh.viewlib.togglebutton;
...
public class ToggleButton extends View{
	...
	/** 开启颜色  按钮边框颜色*/
	private int onColor = Color.parseColor("#00ffffff");
	/** 关闭颜色  按钮边框渐变颜色*/
	private int offBorderColor = Color.parseColor("#bcbcbc");
	/** 灰色带颜色  内部缩放区域颜色*/
	private int offColor = Color.parseColor("#ffffff");
//	private int offColor = Color.parseColor("#bcbcbc");
	/** 手柄颜色*/
	private int spotColor = Color.parseColor("#bcbcbc");
	/** 边框颜色*/
	private int borderColor =  Color.parseColor("#dedede");
	/** 画笔*/
	private Paint paint ;
	/** 开关状态*/
...
//		borderColor = offBorderColor;
	...
	private void takeEffect(boolean animate,boolean toggleOn) {
		if(animate){
...
//			rect.set(spotX - cy, centerY - cy, endX + cy, centerY + cy);
			rect.set(borderWidth + 2, centerY - cy, spotX+cy , centerY + cy);
//			rect.set(0 , 0 , endX + cy, centerY +cy );
			paint.setColor(offColor);
			canvas.drawRoundRect(rect, cy, cy, paint);
		}

//		final float cy = offLineWidth * 0.5f;
//		rect.set(spotMinX - cy, centerY - cy, endX + cy, centerY + cy);
//		paint.setColor(offColor);
//		canvas.drawRoundRect(rect, cy, cy, paint);
		...
		canvas.drawRoundRect(rect, spotR -1, spotR -1, paint);
	...
	private void calculateEffect(final double value) {
		...
		float mapOffLineWidth = (float) SpringUtil.mapValueFromRangeToRange( value, 0, 1, 10, spotSize);
		...
		offBorderColor = Color.rgb(sr, sg, sb);
		int tempColor = Color.parseColor("#bcbcbc");
		int tempColor2 = Color.parseColor("#e5004f");

		 int spotfb = Color.blue(tempColor);
		 int spotfr = Color.red(tempColor);
		 int spotfg = Color.green(tempColor);

		 int spottb = Color.blue(tempColor2);
		 int spottr = Color.red(tempColor2);
		 int spottg = Color.green(tempColor2);
//

		int spotsb ,spotsr ,spotsg;
		spotsb = (int) SpringUtil.mapValueFromRangeToRange( value, 0, 1,spotfb,spottb );
		spotsr = (int) SpringUtil.mapValueFromRangeToRange(value, 0, 1,spotfr,spottr);
		spotsg = (int) SpringUtil.mapValueFromRangeToRange( value, 0, 1,spotfg,spottg );

		spotsb = clamp(spotsb, 0, 255);
		spotsr = clamp(spotsr, 0, 255);
		spotsg = clamp(spotsg, 0, 255);
		spotColor = Color.rgb(spotsr, spotsg, spotsb);
