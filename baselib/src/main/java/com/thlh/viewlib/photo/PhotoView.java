package com.thlh.viewlib.photo;
...
public class PhotoView extends ImageView {
    ...
    private RectF mClip;
    private PhotoViewInfo mPhotoViewInfo;
    ...
    private void initBase() {
        ...
        if (mPhotoViewInfo != null && System.currentTimeMillis() - mInfoTime < MAX_ANIM_FROM_WAITE) {
            animaFrom(mPhotoViewInfo);
        }
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    public PhotoViewInfo getInfo() {
        ...
        return new PhotoViewInfo(rect, local, mImgRect, mWidgetRect, mScale, mDegrees, mScaleType);
    }
     ...
     */
    public void animaFrom(PhotoViewInfo photoViewInfo) {
        ...
        if (isInit) {
            ...
            float scaleX = photoViewInfo.mImgRect.width() / mine.mImgRect.width();
            float scaleY = photoViewInfo.mImgRect.height() / mine.mImgRect.height();
            float scale = scaleX < scaleY ? scaleX : scaleY;

            float ocx = photoViewInfo.mRect.left + photoViewInfo.mRect.width() / 2;
            float ocy = photoViewInfo.mRect.top + photoViewInfo.mRect.height() / 2;

            mAnimaMatrix.reset();
            mAnimaMatrix.postTranslate(-mBaseRect.left, -mBaseRect.top);
            mAnimaMatrix.postTranslate(ocx - mBaseRect.width() / 2, ocy - mBaseRect.height() / 2);
            mAnimaMatrix.postScale(scale, scale, ocx, ocy);
            mAnimaMatrix.postRotate(photoViewInfo.mDegrees, ocx, ocy);
            executeTranslate();
            ...
            mTranslate.withRotate((int) photoViewInfo.mDegrees, 0);

            if (photoViewInfo.mWidgetRect.width() < photoViewInfo.mImgRect.width() || photoViewInfo.mWidgetRect.height() < photoViewInfo.mImgRect.height()) {
                float clipX = photoViewInfo.mWidgetRect.width() / photoViewInfo.mImgRect.width();
                float clipY = photoViewInfo.mWidgetRect.height() / photoViewInfo.mImgRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;

                ClipCalculate c = photoViewInfo.mScaleType == ScaleType.FIT_START ? new START() : photoViewInfo.mScaleType == ScaleType.FIT_END ? new END() : new OTHER();
    ...
    public void animaTo(PhotoViewInfo photoViewInfo, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
            ...
            float tcx = photoViewInfo.mRect.left + photoViewInfo.mRect.width() / 2;
            float tcy = photoViewInfo.mRect.top + photoViewInfo.mRect.height() / 2;
            ...
            // 缩放
            float scaleX = photoViewInfo.mImgRect.width() / mBaseRect.width();
            float scaleY = photoViewInfo.mImgRect.height() / mBaseRect.height();
            float scale = scaleX > scaleY ? scaleX : scaleY;
            ...
            mTranslate.withRotate((int) mDegrees, (int) photoViewInfo.mDegrees, ANIMA_DURING * 2 / 3);

            if (photoViewInfo.mWidgetRect.width() < photoViewInfo.mRect.width() || photoViewInfo.mWidgetRect.height() < photoViewInfo.mRect.height()) {
                float clipX = photoViewInfo.mWidgetRect.width() / photoViewInfo.mRect.width();
                float clipY = photoViewInfo.mWidgetRect.height() / photoViewInfo.mRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;

                final float cx = clipX;
                final float cy = clipY;
                final ClipCalculate c = photoViewInfo.mScaleType == ScaleType.FIT_START ? new START() : photoViewInfo.mScaleType == ScaleType.FIT_END ? new END() : new OTHER();
